...
namespace ValheimCharacterForge.Classes
{
    class Parser
    {
        static public CharacterData CharacterReadData(byte[] data)
        {
            // Read header
            ...
            for (var i = 0; i < numberOfWorlds; i++)
            {
                ...
                var world = new CharacterData.World
                {
            ...
            character.Biomes = new List<CharacterData.Biome>(); // is this the discovered biomes?
            ...
            for (var i = 0; i < numberOfBiomes; i++)
                character.Biomes.Add((CharacterData.Biome)reader.ReadInt32());
            ...
            character.SkinColor = new Vector3
            {
                X = reader.ReadSingle(),
                Y = reader.ReadSingle(),
                Z = reader.ReadSingle()
            };
            character.HairColor = new Vector3
            {
                X = reader.ReadSingle(),
                Y = reader.ReadSingle(),
                Z = reader.ReadSingle()
            ...
            character.Foods = new List<CharacterData.Food>();
            for (var i = 0; i < numberOfConsumedFood; i++)
            {
                var food = new CharacterData.Food
                {
                    Name = reader.ReadString(),
            ...
            character.Skills = new List<CharacterData.Skill>();
            for (var i = 0; i < numberOfSkills; i++)
            {
                var skill = new CharacterData.Skill
                {
                    SkillName = (CharacterData.SkillName)reader.ReadInt32(),
        ...
        static public byte[] CharacterWriteData(CharacterData character)
        {
